...
/** server brainSwap 觸發門檻（tokens）的後備值；正式值由 snapshot 的
 *  system.brainSwapThresholdTokens 帶下來（單一事實來源在 server/brainSwap.ts）。 */
...
export function computeCtxGauge(series: number[], thresholdTokens = SWAP_THRESHOLD_TOKENS): CtxGauge | null {
  ...
  const room = thresholdTokens - baseline;
