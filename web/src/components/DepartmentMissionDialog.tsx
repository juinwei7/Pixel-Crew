...
const statusLabel: Record<DepartmentMission["status"], string> = {
  ...
  executing: t("部門執行中"),
  reviewing: t("專家協作中"),
  needs_attention: t("需要你決定"),
  completed: t("Mission 完成"),
  failed: t("Mission 失敗"),
  cancelled: t("已取消"),
};
...
export function DepartmentMissionDialog({ boss, workers, missions, legacyTasks = [], departmentRecord, onPrepare, onStart, onLoadThread, onMessageDepartment, onResetSessions, resetRequestKey = 0, onCancel, onRetryReview, onApprovePlan, onResolve, onResolveApproval, onAsk, onClose, embedded = false, focusMode = false, missionDetailId = null, focusSection = null, onSelectWorker, composerHost }: Props) {
  ...
  async function continueWithDepartment(submission: CommandSubmission): Promise<string | null> {
    if (!submission.text.trim() && submission.images.length === 0 && submission.documents.length === 0) return t("請輸入訊息或附加檔案");
    ...
    if (departmentRecord && onMessageDepartment) {
      ...
      result = await prepareAndStartDepartmentMission({
        ...
        objective: latestTerminal
          ? t("延續前次 Mission「{objective}」的後續工作：{text}", { objective: latestTerminal.objective, text: submission.text || t("依附件執行後續工作") })
          : submission.text || t("依附加檔案完成交辦工作"),
  ...
  function renderMissionCard(mission: DepartmentMission) {
    ...
    return <div key={mission.id} className="department-chat__exchange">
      ...
      {mission.steps.length > 0 && <ol>{mission.steps.map((step, index) => {
        ...
        return <li key={step.id} className={`mission-step mission-step--${step.status}`}>
          ...
          {step.reviewResult && <div className="mission-step__review">
            ...
            {step.reviewResult.findings.length > 0 && <div><b>{t("發現")}</b><ul>{step.reviewResult.findings.map((item, findingIndex) => <li key={`${item.title}-${findingIndex}`}><strong>{item.severity} · {item.title}</strong>{item.detail ? t("：{detail}", { detail: item.detail }) : ""}{item.file ? (item.line ? t("（{file}:{line}）", { file: item.file, line: String(item.line) }) : t("（{file}）", { file: item.file })) : ""}</li>)}</ul></div>}
            {step.reviewResult.risks.length > 0 && <div><b>{t("風險")}</b><ul>{step.reviewResult.risks.map((item, itemIndex) => <li key={`risk-${itemIndex}`}>{String(item)}</li>)}</ul></div>}
            {step.reviewResult.openQuestions.length > 0 && <div><b>{t("未決問題")}</b><ul>{step.reviewResult.openQuestions.map((item, itemIndex) => <li key={`question-${itemIndex}`}>{String(item)}</li>)}</ul></div>}
            {step.reviewResult.recommendedNextAction && <p><b>{t("建議：")}</b>{step.reviewResult.recommendedNextAction}</p>}
          </div>}
          {!step.reviewResult && step.result && <p>{step.result}</p>}
        </li>;
      })}</ol>}
      {pendingApprovals.length > 0 && <section className="mission-card__approvals" aria-label={t("任務等待核准")}>
        <h4>{t("這項工作需要你的核准")}</h4>
        {pendingApprovals.map(({ workerId, request }) => {
          const worker = workers.find((candidate) => candidate.id === workerId);
          return <article key={request.id} className="mission-card__approval">
            <div><strong>{request.title}</strong><span>{worker?.name ?? t("部門成員")} · {request.category}</span></div>
            {request.reason && <p>{request.reason}</p>}
            {request.command && <code>{request.command}</code>}
            <footer>
              <button type="button" disabled={working || !onResolveApproval} onClick={() => void action(() => onResolveApproval!(mission.id, request.id, "deny"))}>{t("拒絕")}</button>
              <button type="button" disabled={working || !onResolveApproval} onClick={() => void action(() => onResolveApproval!(mission.id, request.id, "allow_once"))}>{t("允許一次")}</button>
              {request.decisions.includes("allow_session") && <button type="button" disabled={working || !onResolveApproval} onClick={() => void action(() => onResolveApproval!(mission.id, request.id, "allow_session"))}>{t("本次任務都允許")}</button>}
            </footer>
          </article>;
        })}
      </section>}
      ...
      <footer>
        ...
        {mission.status === "needs_attention" && current?.kind === "review" && mission.attentionReason !== "plan_approval" && <button type="button" className="collaboration-dialog__primary" onClick={() => void action(() => onRetryReview(mission.id))}>{t("重新 Review")}</button>}
        {mission.correctionCount > 0 && <span>{t("已修正 {done}/{max} 輪", { done: mission.correctionCount, max: mission.maxCorrections })}</span>}
      </footer>
      </article>
    </div>;
  }
  ...
  const newMissionForm = <TaskComposer
    ...
    placeholder={latestTerminal ? t("直接告訴部門要修改或接著完成什麼") : t("直接交辦給 {name}（可附加圖片或文件）", { name: departmentRecord?.name ?? roomName(boss.workspacePath) })}
    submitLabel={t("交辦")}
    busyLabel={t("派工中…")}
    disabled={false}
    working={working}
    ...
    focusMode={focusMode}
    leading={composerHost ? <span className="command-composer__target">{t("部門")}</span> : undefined}
    toolbar={<>{latestTerminal && <div className="department-chat__reply-context"><span>{t("延續")}</span><strong>{latestTerminal.objective}</strong></div>}<div className="department-chat__criteria"><details>
      <summary>{t("＋ 驗收條件")} <span>{t("選填")}</span></summary>
      <div><small>{t("每行一項，最多 8 項")}</small><textarea value={criteria} rows={4} placeholder={t("例如：\n完整測試通過\n最後提交一份部門報告")} onChange={(event) => setCriteria(event.target.value)} /></div>
  ...
  const inner = <>
      <header className="department-chat__header">
        ...
        <div className="department-chat__header-actions">
          <strong className={`department-chat__state ${activeMission ? "department-chat__state--active" : ""}`}>{activeMission ? statusLabel[activeMission.status] : t("可直接交辦")}</strong>
          {departmentRecord && onResetSessions && <button type="button" className="department-chat__fresh" disabled={working} onClick={() => void prepareSessionReset()}>{t("重開全員對話")}</button>}
        </div>
      </header>

      <details className="department-chat__team" open={focusSection === "team"}>
        <summary>{t("{count} 位成員 · 查看職務", { count: department.length })}</summary>
        <section className="mission-dialog__roster" aria-label={t("部門成員")}>
        {department.map((worker) => {
          ...
          const state = worker.busy ? t("執行中") : worker.turns.some((turn) => turn.items.some((item) => item.kind === "approval" && item.status === "pending")) ? t("等待核准") : t("待命");
          return <button key={worker.id} type="button" onClick={() => onSelectWorker?.(worker.id)} disabled={!onSelectWorker} className={assigned ? "mission-dialog__member--assigned" : ""}>
            <span><strong>{worker.name}</strong><small>{worker.persona?.role || (worker.id === boss.id ? t("部門主管") : t("部門成員"))}</small></span>
            <em>{assigned ? t("目前步驟") : state}</em>
          </button>;
        })}
        </section>
      </details>
      ...
      {!activeMission && !latestTerminal && <section className="department-chat__empty">
        ...
        <strong>{t("直接告訴部門要完成什麼")}</strong>
        <p>{t("不需要選模型或再次選部門。送出後，主管會依職務規劃、執行與驗收。")}</p>
      </section>}
      ...
      {resetResults && <section className="department-reset department-reset--results" aria-label={t("工作階段重建結果")}>
        <strong>{t("工作階段重建結果")}</strong>
        <ul>{resetResults.map((result) => <li key={result.workerId} className={result.ok ? "ok" : "failed"}>{result.name} · {result.ok ? t("完成") : result.error || t("失敗")}</li>)}</ul>
        {resetResults.some((result) => !result.ok) && <button type="button" onClick={() => void confirmSessionReset(resetResults.filter((result) => !result.ok).map((result) => result.workerId))}>{t("只重試失敗成員")}</button>}
        <button type="button" onClick={() => setResetResults(null)}>{t("關閉")}</button>
      </section>}
      ...
      {!activeMission && latestTerminal && <section className="department-continuation" aria-label={t("部門報告後續")}>
        ...
        <p>{t("直接輸入即可：由 {name} 判斷這是唯讀提問還是新的後續工作；需要新工作時會建立關聯的 Mission，原任務不會被更動。", { name: boss.name })}</p>
        {followUpTurns.length > 0 && <div className="department-continuation__thread">{followUpTurns.map((turn) => {
          ...
          return <article key={turn.key} className={`department-continuation__turn department-continuation__turn--${turn.status}`}>
            <div><span>{t("你")}</span><p>{turn.command.replace(/^部門追問：/, "")}</p></div>
            <div><span>{boss.name}</span>{answer ? <RichText text={answer} compact /> : <p>{turn.status === "running" ? t("主管正在整理報告脈絡…") : t("這次追問沒有取得可讀回覆")}</p>}</div>
          </article>;
        })}</div>}
        {!composerHost && newMissionForm}
      </section>}
      {activeMission && <section className="department-running-composer" aria-label={t("進行中 Mission 對話")}>
        <p>{t("可以繼續詢問、補充背景或提出修改；重大修改會在安全檢查點重新規劃。")}</p>
        {!composerHost && newMissionForm}
      </section>}
      {!activeMission && !latestTerminal && !composerHost && newMissionForm}
      {legacyRelated.length > 0 && <details className="mission-dialog__legacy">
        <summary>{t("過往單次 NPC 協作 · {count}", { count: legacyRelated.length })}</summary>
        {legacyRelated.map((task) => {
          const target = workers.find((worker) => worker.id === task.targetWorkerId);
          return <article key={task.id}><div><strong>{task.mode === "review" ? "Review" : "Consult"} · {target?.name ?? t("未知 NPC")}</strong><span>{task.status}</span></div><p>{task.objective}</p>{task.result && <small>{task.result.verdict} · {task.result.summary}</small>}</article>;
        })}
      </details>}
    </>;

  const missionTitle = departmentRecord?.name ?? roomName(boss.workspacePath);
  if (embedded) return <>
    <section className="mission-workspace" aria-label={missionTitle}>
      <div className={`handoff-dialog__card mission-dialog__card mission-dialog__card--embedded ${focusMode ? "mission-dialog__card--focus" : ""}`}>{inner}</div>
    </section>
    {composerHost && createPortal(newMissionForm, composerHost)}
  </>;
  return <Modal label={missionTitle} overlayClassName="handoff-dialog mission-dialog" cardClassName={`handoff-dialog__card mission-dialog__card${focusMode ? " mission-dialog__card--focus" : ""}`} closeClassName="handoff-dialog__close" closeLabel={t("關閉部門任務視窗")} onClose={onClose}>{inner}</Modal>;
}
