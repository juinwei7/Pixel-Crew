...
function statusCopy(status: WorkerAttention): string {
  return { approval: t("等待核准"), error: t("執行失敗"), working: t("執行中"), done: t("已完成"), idle: t("待命") }[status];
}
    ...
    const selectContents = <>
      ...
      {!collapsed && <>
        <div className="crew-row__identity">
          {editing ? (
            <input value={draft} maxLength={24} autoFocus className={renameError ? "crew-row__rename--error" : ""} title={renameError ?? t("Enter 儲存，Esc 取消")} onChange={(event) => { setDraft(event.target.value); setRenameError(null); }} onKeyDown={(event) => {
  ...
  return (
    <aside ref={railRef} className={`crew-rail ${collapsed ? "crew-rail--collapsed" : ""}`} aria-hidden={inert || undefined} inert={inert ? "" : undefined}>
      <div className="crew-rail__head">
        ...
        <button type="button" className="crew-rail__add" onClick={onCreate} disabled={workers.length >= MAX_WORKERS} aria-label={t("新增人員")} title={t("新增人員")}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 5v14M5 12h14" /></svg></button>
      </div>
      {!collapsed && searchOpen && <input className="crew-rail__search" value={query} autoFocus placeholder={t("搜尋名字或房間")} aria-label={t("搜尋人員")} onChange={(event) => setQuery(event.target.value)} />}
      ...
      <div ref={listRef} className={`crew-rail__list${drag ? " crew-rail__list--reordering" : ""}`}>
        {pinned && <div className="crew-rail__pinned"><span>{t("目前選取")}</span>{row(pinned, true)}</div>}
        {groups.map(([key, members]) => {
          ...
          return <section key={key} className={`crew-department ${department?.id === selectedDepartmentId ? "crew-department--active" : ""} ${attention ? "crew-department--attention" : ""}`} title={department?.purpose || roomName(members[0]?.workspacePath ?? "")}>
            {!collapsed && <button type="button" className="crew-department__header" disabled={!selectable} aria-current={department?.id === selectedDepartmentId ? "true" : undefined} onClick={() => department && onSelectDepartment?.(department.id)}>
              <span><strong>{department?.name ?? roomName(members[0]?.workspacePath ?? "")}</strong><small>{t("{count} 位 NPC", { count: department?.memberWorkerIds.length ?? members.length })}</small></span>
              {mission ? <em className={`crew-department__mission crew-department__mission--${mission.status}`}>{mission.status === "needs_attention" ? t("需處理") : total ? `${completed}/${total}` : t("規劃中")}</em> : <em>{t("老闆交辦")}</em>}
            </button>}
