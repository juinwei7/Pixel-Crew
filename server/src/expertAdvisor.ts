...
export function expertAdvisorPrompt(input: {
  ...
  const domainRule = proactive
    ? `- In this proactive mode ALWAYS propose — never return need_focus.
- The "Owner workspace" path below is only a filesystem folder label the owner happened to pick; it OFTEN does not reflect what the folder actually contains, and you cannot read its contents. Do NOT treat the folder name as a domain signal or anchor proposals to whatever the name literally says (e.g. a folder named "米線" does not mean the owner runs a noodle shop). Only lean on it if it is an unmistakable, real domain descriptor, and even then keep it a weak hint rather than the theme.
