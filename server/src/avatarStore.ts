...
export class AvatarStore {
  ...
  async save(dataBase64: unknown, mimeType: unknown = "image/png"): Promise<string> {
    ...
    if (!type) throw new AvatarValidationError(t("只接受 PNG 或 GIF 角色圖片"));
    if (typeof dataBase64 !== "string" || !dataBase64 || dataBase64.length > Math.ceil(maxBytes * 4 / 3) + 4) {
      throw new AvatarValidationError(t("角色圖片資料無效或過大"));
    }
    let data: Buffer;
    try {
      data = Buffer.from(dataBase64, "base64");
    } catch {
      throw new AvatarValidationError(t("角色圖片不是有效的 Base64"));
    }
...
export function validateGif(data: Buffer): void {
  if (data.length < 14 || !["GIF87a", "GIF89a"].includes(data.toString("ascii", 0, 6))) {
    throw new AvatarValidationError(t("角色動畫必須是有效的 GIF"));
  }
  const width = data.readUInt16LE(6);
  const height = data.readUInt16LE(8);
  if (!width || !height || width > MAX_GIF_DIMENSION || height > MAX_GIF_DIMENSION) {
    throw new AvatarValidationError(t("GIF 尺寸最大為 {max} × {max} 像素", { max: MAX_GIF_DIMENSION }));
  }
  ...
  while (cursor < data.length) {
    ...
    if (blockType === 0x3b) {
      if (cursor !== data.length) throw new AvatarValidationError(t("GIF 結尾包含多餘資料"));
      sawTrailer = true;
      break;
    }
    if (blockType === 0x21) {
      if (cursor >= data.length) throw new AvatarValidationError(t("GIF extension 不完整"));
      cursor++; // Extension label.
      cursor = skipGifSubBlocks(data, cursor);
      continue;
    }
    if (blockType !== 0x2c || cursor + 9 > data.length) {
      throw new AvatarValidationError(t("GIF block 結構無效"));
    }

    const frameWidth = data.readUInt16LE(cursor + 4);
    const frameHeight = data.readUInt16LE(cursor + 6);
    const framePacked = data[cursor + 8];
    if (!frameWidth || !frameHeight) throw new AvatarValidationError(t("GIF frame 尺寸無效"));
    cursor += 9;
    if (framePacked & 0x80) cursor += 3 * 2 ** ((framePacked & 0x07) + 1);
    if (cursor >= data.length) throw new AvatarValidationError(t("GIF frame 資料不完整"));
    const lzwMinimumCodeSize = data[cursor++];
    if (lzwMinimumCodeSize < 2 || lzwMinimumCodeSize > 8) {
      throw new AvatarValidationError(t("GIF LZW 編碼無效"));
    }
    cursor = skipGifSubBlocks(data, cursor);
    frames++;
    if (frames > MAX_GIF_FRAMES) throw new AvatarValidationError(t("GIF 超過 {max} 個影格上限", { max: MAX_GIF_FRAMES }));
    if (width * height * frames > MAX_GIF_DECODED_PIXELS) {
      throw new AvatarValidationError(t("GIF 超過安全解碼像素預算，請縮短動畫或降低尺寸"));
    }
  }
  if (!sawTrailer || frames === 0) throw new AvatarValidationError(t("GIF 沒有完整的圖片 frame"));
}
...
function skipGifSubBlocks(data: Buffer, start: number): number {
  ...
  while (cursor < data.length) {
    ...
    if (cursor + length > data.length) throw new AvatarValidationError(t("GIF data sub-block 不完整"));
    cursor += length;
  }
  throw new AvatarValidationError(t("GIF data sub-block 缺少結尾"));
}

export function validatePng(data: Buffer): void {
  if (data.length < 33 || !data.subarray(0, 8).equals(PNG_SIGNATURE)) {
    throw new AvatarValidationError(t("角色圖片必須是 PNG"));
  }
  if (data.readUInt32BE(8) !== 13 || data.toString("ascii", 12, 16) !== "IHDR") {
    throw new AvatarValidationError(t("PNG 標頭無效"));
  }
  if (data.readUInt32BE(16) !== AVATAR_WIDTH || data.readUInt32BE(20) !== AVATAR_HEIGHT) {
    throw new AvatarValidationError(t("角色圖片必須正好是 {w} × {h} 像素", { w: AVATAR_WIDTH, h: AVATAR_HEIGHT }));
  }
  const bitDepth = data[24];
  const colorType = data[25];
  if (bitDepth !== 8 || (colorType !== 2 && colorType !== 6)) {
    throw new AvatarValidationError(t("角色 PNG 必須使用 8-bit RGB 或 RGBA 色彩"));
  }
  if (data[26] !== 0 || data[27] !== 0 || data[28] !== 0) {
    throw new AvatarValidationError(t("角色 PNG 必須使用標準非交錯格式"));
  }
  ...
  while (offset < data.length) {
    if (offset + 12 > data.length) throw new AvatarValidationError(t("PNG 區塊不完整"));
    const length = data.readUInt32BE(offset);
    const end = offset + 12 + length;
    if (length > MAX_AVATAR_BYTES || end > data.length) throw new AvatarValidationError(t("PNG 區塊無效"));
    ...
    if (crc32(data.subarray(offset + 4, offset + 8 + length)) !== expectedCrc) {
      throw new AvatarValidationError(t("PNG 校驗失敗"));
    }
    if (!sawHeader) {
      if (type !== "IHDR") throw new AvatarValidationError(t("PNG 首個區塊必須是 IHDR"));
      sawHeader = true;
    } else if (type === "IHDR") {
      throw new AvatarValidationError(t("PNG 包含重複標頭"));
    }
    if (type === "IDAT") idat.push(content);
    if (type === "IEND") {
      if (length !== 0 || end !== data.length) throw new AvatarValidationError(t("PNG 結尾無效"));
      sawEnd = true;
    }
    offset = end;
  }
  if (!sawEnd || idat.length === 0) throw new AvatarValidationError(t("PNG 缺少圖片資料"));
  try {
    ...
    throw new AvatarValidationError(t("PNG 圖片資料無法解壓縮"));
  }
}
