...
- **作戰室（多 Agent 辯論）**：面對高風險問題時，明確選擇「🏛️ 作戰室」才會臨時召集 2–4 位 Claude 短命同儕 NPC（提案／挑戰／權衡，難題再加一個查證立場負責事實查核），平行跑一輪開場與一輪反駁。模型分級隨難度調整（Haiku／Sonnet／Opus），由主持 NPC 彙整成結構化裁決——共識、爭點、優先行動、關鍵指標與圖表——結束後拆除臨時同儕；這會使用 Claude 用量並需數分鐘，辯論歷史之後可回顧或刪除。
